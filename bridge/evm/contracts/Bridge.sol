...
contract Bridge is Ownable {
    ...
    WALYN public token;
    AlynHeaders public headers;
    mapping(address => bool) public signers;
    uint256 public threshold;
    mapping(bytes32 => bool) public processed;
    bool public trustless;
    ...
    constructor(address tokenAddr, address[] memory _signers, uint256 _threshold, address headersAddr) {
        token = WALYN(tokenAddr);
        headers = AlynHeaders(headersAddr);
    ...
    function enableTrustless(bool on) external onlyOwner {
        trustless = on;
    }
    ...
    function mintWithProof(address to, uint256 amount, bytes32 headerHash, bytes32[] calldata proof, bytes32 txHash) external {
        require(trustless, 'trustless disabled');
        require(!processed[txHash], 'processed');
        require(headers.verifyTxInclusion(headerHash, proof, txHash), 'invalid proof');
