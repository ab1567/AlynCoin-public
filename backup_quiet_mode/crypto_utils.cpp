...
quietPrint( "[DEBUG] Attempting to generate Dilithium keys for user: " << username << std::endl);
...
quietPrint( "✅ [DEBUG] Dilithium keys generated and saved to:\n");
    std::cout << "    " << pubPath << "\n    " << privPath << std::endl;
  } else {
quietPrint( "❌ [ERROR] Failed to generate Dilithium keypair for " << username << "\n");
  }
...
quietPrint( "[DEBUG] Attempting to generate Falcon keys for user: " << username << std::endl);
...
quietPrint( "✅ [DEBUG] Falcon keys generated and saved to:\n");
    std::cout << "    " << pubPath << "\n    " << privPath << std::endl;
  } else {
quietPrint( "❌ [ERROR] Failed to generate Falcon keypair for " << username << "\n");
  }
...
quietPrint( "[DEBUG] 🔏 Starting Dilithium signing..." << std::endl);
quietPrint( "[DEBUG] Message size: " << message.size() << " bytes" << std::endl);
quietPrint( "[DEBUG] Private key size: " << privKey.size() << " bytes" << std::endl);
  ...
  if (ret != 0) {
quietPrint( "❌ [ERROR] Dilithium signing failed! ret=" << ret << "\n");
    return {};
  }

  if (siglen == 0 || siglen > signature.size()) {
quietPrint( "❌ [ERROR] Invalid signature length: " << siglen);
              << " (max allowed: " << signature.size() << ")\n";
    return {};
  }

  signature.resize(siglen);
quietPrint( "✅ [DEBUG] Dilithium signature generated. Length: " << siglen << " bytes\n");
  return signature;
}
...
quietPrint( "[DEBUG] 🔍 Verifying Dilithium signature..." << std::endl);
quietPrint( "[DEBUG] Message size: " << message.size() << " bytes" << std::endl);
quietPrint( "[DEBUG] Signature size: " << signature.size() << " bytes" << std::endl);
quietPrint( "[DEBUG] Public key size: " << publicKey.size() << " bytes" << std::endl);

  if (signature.empty() || message.empty() || publicKey.empty()) {
quietPrint( "❌ [ERROR] One or more inputs are empty.\n");
    return false;
  }

  if (publicKey.size() != CRYPTO_PUBLICKEYBYTES) {
quietPrint( "❌ [ERROR] Public key size mismatch! Expected: ");
  ...
  if (result == 0) {
quietPrint( "✅ [DEBUG] Dilithium signature verified successfully.\n");
    return true;
  } else {
quietPrint( "❌ [ERROR] Dilithium signature verification failed!\n");
    return false;
  }
}
// --- Falcon Signing ---
...
quietPrint( "[DEBUG] Falcon Signing Initiated\n");
    ...
    try {
quietPrint( "[DEBUG] Calling Falcon crypto_sign_signature()...\n");
        ...
        if (ret != 0) {
quietPrint( "❌ [ERROR] Falcon signing failed! Return code: " << ret << "\n");
            return {};
        }

        if (actualSigLen == 0 || actualSigLen > maxSigLen) {
quietPrint( "❌ [ERROR] Falcon signature length invalid: " << actualSigLen << "\n");
            return {};
        }

        signature.resize(actualSigLen);
quietPrint( "✅ [DEBUG] Falcon signature generated. Length: " << actualSigLen << "\n");
        return signature;

    } catch (const std::exception &ex) {
quietPrint( "❌ [EXCEPTION] Falcon signing threw exception: " << ex.what() << "\n");
        return {};
    } catch (...) {
quietPrint( "❌ [FATAL] Unknown Falcon signing crash caught!\n");
        return {};
    }
}
...
bool verifyWithFalcon(const std::vector<unsigned char> &message,
                      const std::vector<unsigned char> &signature,
                      const std::vector<unsigned char> &publicKey) {
quietPrint( "[DEBUG] Falcon Verification Initiated\n");
    ...
    if (signature.empty() || message.empty() || publicKey.empty()) {
quietPrint( "❌ [ERROR] Empty input to Falcon verification!\n");
        return false;
    }

    if (publicKey.size() != PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES) {
quietPrint( "❌ [ERROR] Falcon public key size mismatch!\n");
        return false;
    }
    ...
    if (result == 0) {
quietPrint( "✅ [DEBUG] Falcon signature verification PASSED\n");
        return true;
    } else {
quietPrint( "❌ [DEBUG] Falcon signature verification FAILED! Return code: " << result << "\n");
        return false;
    }
}
...
quietPrint( "❌ [ERROR] Loaded Dilithium public key is incorrect length: " << buffer.size() << " (expected " << CRYPTO_PUBLICKEYBYTES << ")\n");
    return {};
  }
  return buffer;
}
...
quietPrint( "❌ [ERROR] Loaded Falcon public key is incorrect length: " << buffer.size() << " (expected " << PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES << ")\n");
    return {};
  }
  return buffer;
}
...
quietPrint( "[DEBUG] Checking and generating PQ keys (Dilithium + Falcon) for: " << username << "\n");
...
quietPrint( "✅ [INFO] Post-quantum keys already exist for user: " << username << "\n");
        return true;
    }
...
quietPrint( "✅ [INFO] Successfully generated post-quantum keys for: "  + username  + "\n");
        return true;
    } else {
quietPrint( "❌ [ERROR] Failed to generate post-quantum keys for: " << username << "\n");
        return false;
    }
}
...
void serializeKeysToProtobuf(const std::string &privateKeyPath, std::string &output) {
    ...
    if (privFile) {
        ...
        if (buffer.empty()) {
quietPrint( "❌ Error: Private key file is empty: " << privateKeyPath << "\n");
            return;
        }
        keyProto.set_private_key(std::string(buffer.begin(), buffer.end()));
        privFile.close();
    } else {
quietPrint( "❌ Error: Cannot open private key file: " << privateKeyPath << "\n");
        return;
    }
    ...
    if (pubFile) {
        ...
        if (buffer.empty()) {
quietPrint( "⚠️ Warning: Public key file is empty: " << publicKeyPath << "\n");
        } else {
            keyProto.set_public_key(std::string(buffer.begin(), buffer.end()));
        }
        pubFile.close();
    } else {
quietPrint( "⚠️ Warning: Cannot open public key file: " << publicKeyPath << "\n");
    }

    if (!keyProto.SerializeToString(&output)) {
quietPrint( "❌ Error: Failed to serialize CryptoKeysProto to string!\n");
    }
}
...
quietPrint( "❌ Error: Failed to parse Protobuf key data!\n");
        return false;
    }
    ...
    if (privStr.empty()) {
quietPrint( "❌ Error: Parsed private key is empty.\n");
        return false;
    }
...
quietPrint( "✅ [DEBUG] Deserialized Private Key Length: " << privateKey.size() << "\n");
quietPrint( "✅ [DEBUG] Deserialized Public Key Length: "  << publicKey.size() << "\n");
...
quietPrint( "✅ [DEBUG] toHex() - Converted " << data.size());
...
std::vector<unsigned char> fromHex(const std::string &hex) {
    ...
    if (hex.empty()) {
quietPrint( "❌ [fromHex] Input is empty.\n");
        return {};
    }

    if (hex.length() % 2 != 0) {
quietPrint( "❌ [fromHex] Hex string has odd length: " << hex.length() << "\n");
        return {};
    }
...
quietPrint( "❌ [fromHex] Hex string too long (" << hex.length() << " chars). Skipping parse.\n");
        return {};
    }
...
quietPrint( "❌ [fromHex] Invalid hex characters: '" << high << "' or '" << low);
                      << "' at position " << i << "\n";
            return {};
        }
...
quietPrint( "✅ [DEBUG] fromHex() - Parsed " << bytes.size());
...
quietPrint( "[DEBUG] Ensuring keys directory exists..." << std::endl);
  ensureKeysDirectory();
...
quietPrint( "[DEBUG] Private key path: " << privateKeyPath << std::endl);
quietPrint( "[DEBUG] Public key path: " << publicKeyPath << std::endl);
...
quietPrint( "[DEBUG] Running private key generation command: " << privCmd);
            << std::endl;
  int privStatus = system(privCmd.c_str());
  if (privStatus != 0) {
quietPrint( "❌ [ERROR] Private key generation failed for user: ");
              << cleanUsername << std::endl;
    return;
  }
quietPrint( "[DEBUG] Private key generated successfully." << std::endl);

quietPrint( "[DEBUG] Running public key extraction command: " << pubCmd);
            << std::endl;
  int pubStatus = system(pubCmd.c_str());
  if (pubStatus != 0) {
quietPrint( "❌ [ERROR] Public key extraction failed for user: ");
              << cleanUsername << std::endl;
    return;
  }
quietPrint( "[DEBUG] Public key generated successfully." << std::endl);

  // Final check to ensure both files now exist
  if (fs::exists(privateKeyPath) && fs::exists(publicKeyPath)) {
quietPrint( "✅ [INFO] Successfully generated keys for user: ");
              << cleanUsername << std::endl;
  } else {
quietPrint( "❌ [ERROR] Key files missing after generation attempt!");
              << std::endl;
  }
}
...
std::string generatePrivateKey(const std::string &user,
  ...
  if (!ctx) {
quietPrint( "❌ Error: Failed to create EVP_PKEY_CTX." << std::endl);
    return "";
  }

  if (EVP_PKEY_keygen_init(ctx) <= 0 ||
      EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, 4096) <= 0) {
quietPrint( "❌ Error: Failed to initialize keygen context or set key length." << std::endl);
    EVP_PKEY_CTX_free(ctx);
    return "";
  }

  if (EVP_PKEY_keygen(ctx, &pkey) <= 0) {
quietPrint( "❌ Error: Key generation failed." << std::endl);
    EVP_PKEY_CTX_free(ctx);
    return "";
  }
  EVP_PKEY_CTX_free(ctx);
  ...
  if (!bio) {
quietPrint( "❌ Error: Failed to create file BIO for path: " << privateKeyPath << std::endl);
...
quietPrint( "❌ Error: Failed to write private key to file without encryption." << std::endl);
    BIO_free(bio);
    EVP_PKEY_free(pkey);
    return "";
  }
  ...
  if (!bio) {
quietPrint( "❌ Error: Failed to create memory BIO." << std::endl);
    EVP_PKEY_free(pkey);
    return "";
  }

  if (!PEM_write_bio_PrivateKey(bio, pkey, nullptr, nullptr, 0, nullptr, nullptr)) {
quietPrint( "❌ Error: Failed to write private key to memory without encryption." << std::endl);
    BIO_free(bio);
    EVP_PKEY_free(pkey);
    return "";
  }
...
std::string getPublicKey(const std::string &user) {
  ...
  if (!fs::exists(privateKeyPath)) {
quietPrint( "⚠️ [WARNING] Private key missing for " << user);
...
quietPrint( "❌ [ERROR] Failed to generate public key for " << user);
              << "!\n";
    return "";
  }
  ...
  std::ifstream pubFile(publicKeyPath);
  if (!pubFile) {
quietPrint( "❌ [ERROR] Failed to open generated public key file!\n");
    return "";
  }
...
EVP_PKEY *loadPrivateKey(const std::string &privateKeyPath) {
  if (!fs::exists(privateKeyPath)) {
quietPrint( "❌ [ERROR] Private key file missing: " << privateKeyPath);
              << "\n";
    return nullptr;
  }
...
quietPrint( "❌ [ERROR] Cannot open private key file: " << privateKeyPath);
  ...
  if (!privateKey) {
quietPrint( "❌ [ERROR] Failed to parse private key!\n");
  }
  return privateKey;
}
...
quietPrint( "✅ Miner keys verified and exist.\n");
  }
}
...
quietPrint( "[DEBUG] Signing message: " << message);
  ...
  if (isFilePath) {
    if (!fs::exists(privateKeyPath)) {
quietPrint( "❌ [ERROR] Private key file not found: " << privateKeyPath << std::endl);
      return "";
    }
...
quietPrint( "❌ [ERROR] Private key is empty!\n");
    return "";
  }
...
bool verifyMessage(const std::string &publicKeyPath,
  ...
  if (!fs::exists(publicKeyPath)) {
quietPrint( "❌ [ERROR] Public key file not found: " << publicKeyPath << std::endl);
    return false;
  }
  ...
  if (publicKeyPath.find("falcon") != std::string::npos) {
    ...
    if (!pkey) {
quietPrint( "❌ [ERROR] Failed to parse public key." << std::endl);
      return false;
    }
