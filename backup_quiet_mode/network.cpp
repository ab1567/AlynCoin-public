...
Network::Network(unsigned short port, Blockchain* blockchain, PeerBlacklist* blacklistPtr)
    ...
    if (!blacklistPtr) {
quietPrint( "❌ [FATAL] PeerBlacklist is null! Cannot initialize network.\n");
    ...
    try {
        ...
        if (ec) {
quietPrint( "⚠️ [Network] Failed to set socket option: " << ec.message() << "\n");
        }

        acceptor.bind(boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), port), ec);
        if (ec) {
quietPrint( "❌ [Network Bind Error] bind failed on port " << port);
                      << ": " << ec.message() << "\n";
quietPrint( "❌ Failed to bind Network on port " << port);
        ...
        if (ec) {
quietPrint( "❌ [Network Listen Error] " << ec.message() << "\n");
            return;
        }
...
quietPrint( "❌ [Network Exception] " << ex.what() << "\n");
    }
}
...
quietPrint( "✅ Network instance cleaned up safely." << std::endl);
  } catch (const std::exception &e) {
quietPrint( "❌ Error during Network destruction: " << e.what());
              << std::endl;
  }
}
//
...
quietPrint( "📡 New peer connected: " << peerAddr << std::endl);
...
quietPrint( "❌ Miner private keys not found or invalid!" << std::endl);
          continue;
        }
...
quietPrint( "❌ Mined block failed validation or signature check!" << std::endl);
        }
      }
    }
  }).detach();
...
quietPrint( "📡 Sent message: " << message << std::endl);
  } catch (const std::exception &e) {
  }
}
//
void Network::sendMessageToPeer(const std::string &peer,
  ...
  try {
    ...
    boost::asio::write(socket, boost::asio::buffer(message + "\n"));
quietPrint( "📡 Sent message to peer " << peer << ": " << message);
              << std::endl;
  } catch (const std::exception &e) {
  }
}
...
quietPrint( "❌ [ERROR] Failed to broadcast transaction to ");
...
quietPrint( "⚠️ [WARNING] No peers available for sync!\n");
        return;
    }
...
quietPrint( "📡 [DEBUG] Requesting blockchain sync from " << peer << "...\n");
        std::string base64Response = requestBlockchainSync(peer);

        if (base64Response.empty()) {
quietPrint( "❌ [ERROR] No data received from " << peer << "!\n");
            continue;
        }

quietPrint( "📡 [DEBUG] Raw Blockchain Data from Peer (" << peer);
...
quietPrint( "❌ [ERROR] Failed to parse blockchain data from peer: " << peer << "\n");
            continue;
        }

        auto safeBinaryToString = [](const std::vector<uint8_t> &vec, size_t maxLen = 100000) -> std::string {
            if (vec.size() > maxLen) {
quietPrint( "❌ [syncWithPeers] zkProof too large (" << vec.size() << " bytes). Skipping.\n");
                return "";
            }
        ...
        for (const auto &blk : tempChain.getChain()) {
            ...
            if (proofStr.empty()) {
                zkValid = false;
quietPrint( "❌ [ERROR] Invalid or oversized zk-STARK proof during sync from peer: " << peer << "\n");
                break;
            }
            ...
            if (!WinterfellStark::verifyProof(proofStr, blk.getHash(),
                ...
                zkValid = false;
quietPrint( "❌ [ERROR] Invalid zk-STARK proof detected in synced block from: " << peer << "\n");
                break;
            }
            ...
            if (!dilPass || !falPass) {
                zkValid = false;
quietPrint( "❌ [ERROR] Signature verification failed in block from: " << peer << "\n");
                break;
            }
        }

        if (zkValid && tempChain.getBlockCount() > blockchain.getBlockCount()) {
quietPrint( "✅ Valid zk-STARK chain received. Merging...\n");
            blockchain.mergeWith(tempChain);
            blockchain.saveToDB();
        } else if (!zkValid) {
quietPrint( "❌ Rejected chain due to invalid zk-STARK proofs or signatures.\n");
        } else {
quietPrint( "✅ Local chain is up-to-date. No merge needed.\n");
        }
...
quietPrint( "✅ Connected to peer: " << peerKey << "\n");

    } catch (const std::exception &e) {
quietPrint( "❌ Failed to connect to peer: " << peerKey);
...
quietPrint( "📡 Requesting peer list from all known peers..." << std::endl);
}
//
...
quietPrint( "✅ Replacing local chain with valid, longer chain from " << senderIP << "\n");
        mainChain.replaceChain(tempChain.getChain());
        mainChain.saveToDB();
    } else {
quietPrint( "❌ Rejected chain from " << senderIP << ": invalid or shorter.\n");
    }
}
// network node
...
quietPrint( "✅ Connected to new peer: " << peerKey << "\n");
...
quietPrint( "❌ Error connecting to node: " << e.what() << std::endl);
        return false;
    }
}
...
std::vector<std::string> Network::discoverPeers() {
    ...
    if (!file) {
quietPrint( "⚠️ [WARNING] No known peers found. Bootstrap required!" << std::endl);
        return peers;
    }
...
quietPrint( "⚠️ [WARNING] Invalid port for peer: " << peer << "\n");
        continue;
      }
    }

    if (ip == "127.0.0.1" && port == this->port) {
quietPrint( "⚠️ Skipping self in discovered peers: " << peer << "\n");
      continue;
    }
...
quietPrint( "📡 [DEBUG] Periodic sync request to " << peerAddr << "\n");

                std::string response = requestBlockchainSync(peerAddr);
                if (response.empty()) {
quietPrint( "⚠️ [WARNING] Skipping peer " << peerAddr << " due to empty response.\n");
                    continue;
                }

                std::string decoded = Crypto::base64Decode(response);
                if (decoded.empty()) {
quietPrint( "⚠️ [WARNING] Invalid base64 response from peer: " << peerAddr << "\n");
                    continue;
                }
...
quietPrint( "❌ [ERROR] Failed to parse blockchain from peer: " << peerAddr << "\n");
                    continue;
                }
                ...
                for (const auto &blk : tempChain.getChain()) {
                    std::string proofStr(blk.getZkProof().begin(), blk.getZkProof().end());
                    if (!WinterfellStark::verifyProof(proofStr, blk.getHash(), blk.getPreviousHash(), blk.getTxRoot())) {
quietPrint( "❌ [ERROR] Invalid zk-STARK proof in block from: " << peerAddr << "\n");
                        valid = false;
                        break;
                    }
                }
...
quietPrint( "✅ Periodic sync accepted. Replacing local chain with chain from " << peerAddr << "\n");
                    localChain.replaceChain(tempChain.getChain());
                    localChain.saveToDB();
                } else if (!valid) {
quietPrint( "❌ Rejected chain from " << peerAddr << " due to invalid zk-STARK proofs.\n");
                } else {
...
quietPrint( "[DEBUG] Incoming Data (" << senderIP << ") Length: " << data.length() << "\n");
quietPrint( "[DEBUG] First 200 bytes: " << data.substr(0, 200) << "\n");

    // 🔄 Sync request
    if (data == "REQUEST_BLOCKCHAIN") {
quietPrint( "📡 [INFO] REQUEST_BLOCKCHAIN received from " << senderIP << "\n");
...
quietPrint( "✅ Sent blockchain data back to peer " << senderIP << "\n");
            } else {
quietPrint( "❌ Failed to serialize blockchain for peer " << senderIP << "\n");
            }
        } else {
quietPrint( "❌ [ERROR] Could not resolve peer socket for sender: " << senderIP << "\n");
        }
        return;
    }
    ...
    if (data.rfind(blockPrefix, 0) == 0) {
        ...
        if (base64.empty()) {
quietPrint( "❌ [ERROR] Empty BLOCK_DATA payload from: " << senderIP << "\n");
            return;
        }
...
quietPrint( "❌ [ERROR] Exception during base64Decode for peer " << senderIP << ": " << e.what() << "\n");
            return;
        }

        if (serialized.empty()) {
quietPrint( "❌ [ERROR] Base64 decode failed or returned empty for peer: " << senderIP << "\n");
            return;
        }

        alyncoin::BlockProto proto;
        if (!proto.ParseFromString(serialized)) {
quietPrint( "❌ [ERROR] Failed to parse Protobuf Block from peer: " << senderIP << "\n");
            return;
        }

quietPrint( "[DEBUG] Parsed Protobuf Block from peer " << senderIP);
                  << " | Index: " << proto.index()
...
quietPrint( "📥 Received BLOCK_DATA from " << senderIP << ", attempting to validate...\n");
            handleNewBlock(newBlock);
        } catch (const std::exception &e) {
quietPrint( "❌ [Exception] Error during block parse/validation: " << e.what() << "\n");
            return;
        }
...
quietPrint( "❌ [ERROR] Failed to parse blockchain data from peer: " << senderIP << "\n");
            return;
        }
        ...
        for (const auto &blk : mainChain.getChain()) {
            if (blk.getIndex() == 0) {
quietPrint( "⚠️ [WARNING] Genesis block zk-STARK check: zkProof size = " << blk.getZkProof().size() << "\n");
                if (blk.getZkProof().empty()) {
quietPrint( "❌ [ERROR] Genesis block has empty zkProof after sync!\n");
                    validChain = false;
                    break;
                }
                continue;
...
quietPrint( "❌ Failed to re-add block from synced chain. Aborting chain adoption.\n");
                    return;
                }
            }
...
quietPrint( "[INFO] ✅ Blockchain fully adopted from peer " << senderIP);
                      << ". New height: " << localChain.getHeight() << "\n";
        } else {
quietPrint( "[ERROR] ❌ Blockchain rejected due to verification failure.\n");
        }
        return;
    }
...
quietPrint( "[DEBUG] Ignored unrecognized non-prefixed message from " << senderIP << "\n");
        }
    } catch (const std::exception &e) {
...
void Network::broadcastBlock(const Block &block) {
    ...
    for (const auto &peer : peerSockets) {
        ...
        if (socket && socket->is_open()) {
            try {
                boost::asio::write(*socket, boost::asio::buffer(message + "\n"));
quietPrint( "📡 [LIVE BROADCAST] Block sent to: " << peer.first << "\n");
            } catch (const std::exception &e) {
quietPrint( "❌ Failed to send block to " << peer.first << ": " << e.what() << "\n");
            }
        }
    }
}
...
quietPrint( "✅ Peer validated: " << peer << std::endl);
  return true;
}
...
quietPrint( "❌ [ERROR] Block PoW check failed!\n");
        return;
    }
...
quietPrint( "❌ [ERROR] Invalid zk-STARK proof detected in new block!\n");
        return;
    }
...
quietPrint( "⚠️ [Node] Ignoring duplicate or old block. Index: ");
                  << newBlock.getIndex() << ", Tip Index: " << expectedIndex - 1 << "\n";
        return;
    }

    if (newBlock.getIndex() > expectedIndex) {
quietPrint( "⚠️ [Node] Received future block. Index: ");
...
quietPrint( "❌ Invalid block received! Rejecting.\n");
        return;
    }

    // 🔍 Debug signatures and keys
debugPrint( "[DEBUG] Block hash: " << newBlock.getHash() << "\n");
quietPrint( "[DEBUG] Dilithium Signature Length: " << newBlock.getDilithiumSignature().size() << "\n");
quietPrint( "[DEBUG] Falcon Signature Length: " << newBlock.getFalconSignature().size() << "\n");
quietPrint( "[DEBUG] Public Key Dilithium Length: " << newBlock.getPublicKeyDilithium().size() << "\n");
quietPrint( "[DEBUG] Public Key Falcon Length: " << newBlock.getPublicKeyFalcon().size() << "\n");
    ...
    if (!Crypto::verifyWithDilithium(msgBytes, sigDil, pubDil)) {
quietPrint( "❌ Dilithium signature verification failed!\n");
        return;
    }
    ...
    if (!Crypto::verifyWithFalcon(msgBytes, sigFal, pubFal)) {
quietPrint( "❌ Falcon signature verification failed!\n");
        return;
    }
...
quietPrint( "❌ [ERROR] Peer socket not found or not open: " << peer << "\n");
        return false;
    }
    ...
    try {
        ...
        if (message.back() != '\n') message += "\n";  // Ensure newline
quietPrint( "📡 [DEBUG] Sending message to " << peer << ": ");
                  << message.substr(0, 200) << "...\n";
        boost::asio::write(*it->second, boost::asio::buffer(message));
        return true;
    } catch (const std::exception &e) {
quietPrint( "❌ [ERROR] Failed to send data to " << peer << ": " << e.what() << "\n");
        return false;
    }
}
...
std::string Network::requestBlockchainSync(const std::string &peer) {
    if (peerSockets.find(peer) == peerSockets.end()) {
quietPrint( "❌ [ERROR] Peer not found: " << peer << "\n");
        return "";
    }

quietPrint( "📡 Requesting blockchain sync from: " << peer << "\n");

    if (!sendData(peer, "REQUEST_BLOCKCHAIN")) {
quietPrint( "❌ Failed to send sync request to " << peer << "\n");
        return "";
    }

    std::string response = receiveData(peer);
    if (response.empty()) {
quietPrint( "❌ [ERROR] No blockchain data received from " << peer << "!\n");
        return "";
    }

quietPrint( "📥 Received blockchain sync data from " << peer);
...
quietPrint( "❌ [ERROR] Unexpected blockchain sync format from " << peer << "\n");
    return "";
}
...
quietPrint( "❌ [ERROR] Server failed to start: " << e.what() << "\n");
quietPrint( "⚠️ Try using a different port or checking if another instance ");
                 "is running.\n";
  }
}
...
quietPrint( "❌ [ERROR] Peer not found: " << peer << std::endl);
            return "";
        }
...
quietPrint( "❌ [ERROR] Error reading data from " << peer << ": ");
...
quietPrint( "📥 [DEBUG] Received Data from " << peer << ": ");
                  << receivedData.substr(0, 200) << "...\n";
        return receivedData;
    } catch (const std::exception &e) {
quietPrint( "❌ [EXCEPTION] Exception in receiveData: " << e.what() << std::endl);
        return "";
    }
}
...
quietPrint( "⚠️ Exception in handlePeer: " << e.what() << "\n");
    }
}
...
void Network::loadPeers() {
  ...
  if (!file.is_open()) {
quietPrint( "⚠️ [WARNING] peers.txt not found. Attempting auto-discovery...\n");
    scanForPeers();
    return;
  }
...
quietPrint( "⚠️ Skipping self-peer: " << line << "\n");
      continue;
    }
...
quietPrint( "🔍 Scanning for active AlynCoin nodes..." << std::endl);
...
quietPrint( "✅ Found & connected to: " << peer << std::endl);
            savePeers();  // Save only after successful connection
        }
    }

    if (peerSockets.empty()) {
quietPrint( "⚠️ No active peers found. Will retry periodically." << std::endl);
    }
}
...
quietPrint( "⚠️ Warning: Failed to backup peers.txt: " << e.what());
                << "\n";
    }
  }
  ...
  if (!file.is_open()) {
quietPrint( "❌ Error: Unable to open peers.txt for saving!" << std::endl);
    return;
  }
  ...
  file.close();
quietPrint( "✅ Peer list saved successfully. Total peers: ");
...
quietPrint( "⚠️ Warning: Blockchain is empty! No block to send.\n");
        return;
    }
...
quietPrint( "📡 Sent latest block to " << peerIP);
...
quietPrint( "❌ [ERROR] Failed to serialize blockchain for sync.\n");
        return;
    }
...
quietPrint( "📡 Sending BLOCKCHAIN_DATA to " << peer);
...
void Network::cleanupPeers() {
  ...
  for (const auto &peer : peerSockets) {
    try {
      if (!peer.second || !peer.second->is_open()) {
quietPrint( "⚠️ Peer socket closed: " << peer.first << "\n");
      ...
      if (ec) {
quietPrint( "⚠️ Failed to ping peer: " << peer.first);
                  << " - Marking as inactive.\n";
        inactivePeers.push_back(peer.first);
      } else {
quietPrint( "✅ Peer active: " << peer.first << "\n");
      }
    } catch (const std::exception &e) {
quietPrint( "⚠️ Exception checking peer " << peer.first << ": ");
                << e.what() << "\n";
      inactivePeers.push_back(peer.first);
    }
  }
...
void Network::receiveRollupBlock(const std::string &data) {
  if (data.empty()) {
quietPrint( "❌ [ERROR] Received empty rollup block data!\n");
    return;
  }
