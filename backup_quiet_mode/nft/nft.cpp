...
quietPrint( "✅ [ZK] NFT zk-STARK proof generated. Size: " << zkStarkProof.size() << " bytes\n");
}
...
quietPrint( "[DEBUG] Submitting metadata tx: " << cmd << std::endl);

        FILE* pipe = popen(cmd.c_str(), "r");
        if (!pipe) {
quietPrint( "❌ Failed to open subprocess.\n");
            return false;
        }
...
quietPrint( "[DEBUG] Metadata TX subprocess exited with code: " << exitCode << "\n");
        return success;
    };
...
quietPrint( "⚠️ L1 transaction failed or timed out. Trying L2...\n");
...
bool reMintNFT(const std::string& creator,
    ...
    if (!submitted) {
quietPrint( "❌ Failed to submit transaction.\n");
        return false;
    }
...
bool NFT::exportToFile(const std::string& filename) const {
    ...
    if (!out) {
quietPrint( "❌ Failed to write file: " << fname << "\n");
        return false;
    }
    out << toJSON();
    out.close();
quietPrint( "✅ NFT exported to " << fname << "\n");
    return true;
}
//
// 🔓 Utility: hash helper
...
quietPrint( "⚠️ Output timeout reached.\n");
                break;
            }
        }
...
void exportNFTtoFile(const std::string& filename, const std::string& metadataHash,
    ...
    if (!out) {
quietPrint( "❌ Failed to export .alynft file\n");
        return;
    }
...
quietPrint( "✅ Exported re-mint info to " << filename << "\n");
}
