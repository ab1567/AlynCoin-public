...
void interactiveMenu() {
	...
	if (creator.empty()) {
	    ...
	    if (creator.empty()) {
quietPrint( "❌ Aborting mint. Wallet address required.\n");
	        continue;
	    }
	}
...
quietPrint( "❌ Metadata transaction failed.\n");
	        continue;
	    }
...
quietPrint( "❌ Failed to verify or save NFT.\n");
	        continue;
	    }
...
quietPrint( "❌ NFT not found.\n");
	    ...
	    if (current.empty()) {
quietPrint( "❌ Aborting. Wallet address required.\n");
	        continue;
	    }
	}
	    if (nft.owner != current || nft.revoked) {
quietPrint( "❌ Not the owner or NFT is revoked.\n");
	        continue;
	    }
	    ...
	    if (!nft.verifySignature() || !NFTStorage::saveNFT(nft, DB::getInstance()->getRawDB())) {
quietPrint( "❌ Failed to verify or save transfer.\n");
	        continue;
	    }

quietPrint( "✅ NFT transferred.\n");
	}
...
quietPrint( "⚠️  Re-minting is an advanced feature used for updating NFT metadata with version tracking.\n");
	    std::string confirm;
	    std::cout << "Do you want to proceed? (y/n): ";
	    std::getline(std::cin, confirm);
	    if (confirm != "y" && confirm != "Y") {
quietPrint( "❌ Re-minting canceled.\n");
	        continue;
	    }
...
quietPrint( "❌ NFT not found in local DB.\n");
	        continue;
	    }

	    std::string currentUser = getLoadedWalletAddress();
	    if (nft.owner != currentUser || nft.revoked) {
quietPrint( "❌ You are not the owner of this NFT or it is revoked.\n");
	        continue;
	    }
...
quietPrint( "❌ Failed to verify or save updated NFT.\n");
	        continue;
	    }
...
quietPrint( "✅ Asset bundled.\n");
           ...
           if (opt == 1) {
              std::cout << "Enter expiry timestamp: ";
               std::cin >> nft.expiry_timestamp;
               std::cin.ignore();
quietPrint( "✅ Expiry set.\n");
            } else {
                nft.revoked = true;
quietPrint( "✅ NFT revoked.\n");
            }
            NFTStorage::saveNFT(nft, DB::getInstance()->getRawDB());
        }
...
quietPrint( "✅ Encrypted metadata stored.\n");
        }
...
quietPrint( "⚠️ No encrypted metadata found for this NFT.\n");
                continue;
            }
...
quietPrint( "❌ Decryption failed: " << e.what() << "\n");
            }
        }
...
quietPrint( "❌ Decryption failed: " << e.what() << "\n");
    ...
    if (cmd == "verifyhash" && argc >= 3) {
        ...
        if (!file) {
quietPrint( "❌ File not found: " << filepath << "\n");
            return 1;
        }
...
quietPrint( "✅ NFT found for file!\n" << nft.toJSON() << "\n");
                return 0;
            }
        }

quietPrint( "❌ No NFT found matching the file hash.\n");
        return 1;
    }

quietPrint( "❌ Unknown or unsupported command. Use interactive mode.\n");
    return -1;
}
} // namespace NFTCLI
