...
quietPrint( "⚠️ Missing Dilithium keys. Generating...\n");
...
quietPrint( "⚠️ Missing Falcon keys. Generating...\n");
...
Wallet::Wallet(const std::string& privateKeyPath, const std::string& keyDirectoryPath, const std::string& walletName)
    ...
    if (derived != address) {
quietPrint( "⚠️ Warning: Loaded public key doesn't match provided address.\n");
    }
...
std::string Wallet::signWithPrivateKey(const std::string& message) {
    if (privateKey.empty()) {
quietPrint( "❌ RSA private key missing.\n");
        return "";
    }
...
bool Wallet::saveKeys(const std::string& privKey, const std::string& pubKey) {
    ...
    if (!privFile || !pubFile) {
quietPrint( "❌ Error: Failed to save wallet keys!\n");
        return false;
    }
    privFile << privKey;
    pubFile << pubKey;
