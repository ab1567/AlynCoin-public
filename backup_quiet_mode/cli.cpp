...
void quietPrint(const std::string& msg) {
    if (!QUIET_MODE) std::cout << msg;
}
void quietError(const std::string& msg) {
    if (!QUIET_MODE) std::cerr << msg;
}
...
int main(int argc, char **argv) {
	...
	auto getBlockchain = [&]() -> Blockchain& {
	    if (skipDB) {
quietPrint( "⚠️ CLI is running in --nodb mode.\n");
	        return Blockchain::getInstanceNoDB();
	    }
	    if (skipNet) {
quietPrint( "⚠️ CLI is running in --nonetwork mode (DB OK).\n");
   ...
   // mineonce <minerAddress>
    if (argc >= 3 && std::string(argv[1]) == "mineonce") {
        ...
        if (!b.loadFromDB()) {
quietPrint( "❌ Could not load blockchain from DB.\n");
            return 1;
        }
...
quietPrint( "⚠️ Mining failed or no valid transactions.\n");
            }
...
quietPrint( "✅ YES: " << static_cast<uint64_t>(p.yes_votes) << " | ❌ NO: " << static_cast<uint64_t>(p.no_votes) << "\n");
...
quietPrint( "❌ Wallet creation failed: " << e.what() << "\n");
            return 1;
        }
        std::exit(0);
    }
...
quietPrint( "❌ Wallet key files not found for: " << name << "\n");
            return 1;
        }
...
quietPrint( "❌ Wallet load failed: " << e.what() << "\n");
            return 1;
        }
        std::exit(0);
    }
...
quietPrint( "❌ Invalid arguments for sendl1/sendl2\n");
            return 1;
        }
...
quietPrint( "❌ Failed to submit proposal.\n");
        }
        std::exit(0);
    }
...
quietPrint( "✅ Vote cast!\n");
        } else {
quietPrint( "❌ Failed to vote.\n");
        }
        std::exit(0);
    }
...
quietPrint( "🔍 Loading blockchain from DB...\n");
	    b.loadFromDB();
	    b.reloadBlockchainState();
...
quietPrint( "❌ Invalid --last argument.\n");
                return 1;
            }
...
quietPrint( "❌ Invalid address or --last count.\n");
        return 1;
    }
    ...
    if (hashes.empty()) {
quietPrint( "⚠️ No transactions found for " << addr << ".\n");
        return 0;
    }
...
quietPrint( "❌ Mining failed or returned empty block.\n");
         return 1;
     }
    ...
    if (cmdIndex < argc) {
        ...
        if (!known) {
quietPrint( "❌ Unknown or unsupported command: " << cmd << "\n");
            return 1;
        }
        std::exit(0);
    }
...
quietPrint( "❌ Invalid address. Must be alphanumeric and <= 40 characters.\n");
          break;
        }
        std::string privPath = keyDir + newAddress + "_private.pem";
        if (std::filesystem::exists(privPath)) {
quietPrint( "⚠️ Wallet already exists. Use option 2 to load.\n");
          break;
        }
...
quietPrint( "❌ Wallet creation failed: " << e.what() << std::endl);
          wallet = nullptr;
        }
...
quietPrint( "❌ Wallet loading failed: " << e.what() << std::endl);
        wallet = nullptr;
      }
      break;
    }
    ...
    case 4: {  // 🔁 Send L1 Transaction
      if (!wallet) {
quietPrint( "❌ Load or create a wallet first!\n");
        break;
      }
...
debugPrint( "[DEBUG] signTransaction() called for sender: " << sender << "\n");
      tx.signTransaction(dilPriv.privateKey, falPriv.privateKey);

      if (!tx.getSignatureDilithium().empty() && !tx.getSignatureFalcon().empty()) {
        blockchain.addTransaction(tx);
quietPrint( "📦 [DEBUG] Saving pending transactions to RocksDB...\n");
...
quietPrint( "❌ Rollup Block creation failed: Proof invalid.\n");
    }
...
quietPrint( "❌ Invalid fund allocation details.\n");
            break;
        }
    }
...
quietPrint( "❌ Failed to submit proposal.\n");
   ...
   case 13: {
    ...
    if (weight == 0) {
quietPrint( "⚠️ You have no voting weight (0 balance).\n");
        break;
    }

    if (DAO::castVote(proposal_id, voteYes, weight)) {
quietPrint( "✅ Vote cast successfully! Weight: " << weight << "\n");
    } else {
quietPrint( "❌ Failed to cast vote.\n");
    }
...
quietPrint( "❌ Failed to finalize proposal.\n");
        }
        break;
    }
...
quietPrint( "❌ Recursive Rollup failed validation.\n");
        }
    }
...
quietPrint( "❌ Mid Activity Rollup failed validation.\n");
        }
    }
...
quietPrint( "❌ Idle Rollup failed validation.\n");
        }
    } else {
    ...
    if (!didRollup) {
quietPrint( "⚠️ Rollup not generated. All thresholds unmet or no pending L2 transactions.\n");
    }
    break;
	}
	case 19: {
...
quietPrint( "❌ Load or create a wallet first!\n");
    ...
    if (count <= 0) {
quietPrint( "❌ Invalid transaction count.\n");
        break;
    }
