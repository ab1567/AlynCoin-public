...
quietPrint( "❌ Invalid amount format.\n");
        return;
    }
...
quietPrint( "❌ Invalid duration format.\n");
        return;
    }
...
quietPrint( "❌ Failed to create swap.\n");
    }
}
...
quietPrint( "❌ Invalid amount or duration format.\n");
                return 1;
            }
            ...
            auto swap = manager.getSwap(args["id"]);
            if (!swap) {
quietPrint( "❌ Swap not found.\n");
                return 1;
            }
            if (verifySwapSignature(*swap)) {
quietPrint( "✅ Signature Verification PASSED\n");
            } else {
quietPrint( "❌ Signature Verification FAILED\n");
            }

        } else if (cmd == "verifyproof") {
            auto swap = manager.getSwap(args["id"]);
            if (!swap || !swap->zkProof) {
quietPrint( "❌ No zk-STARK proof available.\n");
                return 1;
            }
...
quietPrint( (valid ? "✅ zk-STARK Proof Verified\n" : "❌ zk-STARK Proof Invalid\n"));
