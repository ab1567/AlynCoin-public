    ...
    try {
      ...
      if (method == "balance") {
        ...
        if (name.empty()) {
          ...
          } while (std::filesystem::exists(DBPaths::getKeyDir() + name +
                                           "_private.pem"));
        ...
        if (!output.contains("error")) {
          try {
            // Generate keys, encrypting the private key when a passphrase is
            // provided
      ...
      // Rollup Block
        ...
        if (l2Transactions.empty()) {
          ...
          // Execute pending L2 txs to get roots and receipts commitment
          std::vector<L2Tx> batch;
          for (const auto &t : l2Transactions) {
          ...
          if (blockchain->isRollupBlockValid(rollup)) {
            ...
            output = {{"result",
                       {{"hash", rollup.getHash()},
                        {"prev_l2_root", prevRoot},
                        {"post_l2_root", postRoot},
                        {"receipts_commitment", rcCommit}}}};
          } else {
      ...
      // Generate recursive zk-STARK proof for last N transactions
      else if (method == "recursiveproof") {
        std::string addr = params.at(0);
        int count = params.at(1);
        blockchain->loadFromDB();
        blockchain->reloadBlockchainState();

        std::vector<std::string> hashes;
        auto blocks = blockchain->getAllBlocks();
        for (auto it = blocks.rbegin(); it != blocks.rend() &&
                                     static_cast<int>(hashes.size()) < count;
             ++it) {
          for (const auto &tx : it->getTransactions()) {
            if (static_cast<int>(hashes.size()) >= count)
              break;
        ...
        if (hashes.empty()) {
          output = {{"error", "No transactions found"}};
        } else {
          std::string outFile =
              DBPaths::getHomePath() +
              "/.alyncoin/recursive_proof_tmp.json";
          generateRecursiveProofToFile(hashes, addr, hashes.size(), outFile);
          std::ifstream in(outFile);
          if (!in) {
            output = {{"error", "Proof generation failed"}};
          } else {
            std::stringstream ss;
            ss << in.rdbuf();
            output = {{"result", ss.str()}};
          }
        }
      }
      ...
      // DAO View all proposals
      else if (method == "dao-view") {
        auto proposals = DAOStorage::getAllProposals();
        nlohmann::json arr = nlohmann::json::array();
        for (const auto &p : proposals) {
          arr.push_back({{"id", p.proposal_id},
                        {"creator", p.proposer_address},
                        {"desc", p.description},
      ...
      // ===================== NFT SPACE =====================
        ...
        static const uint8_t wasm[] = {
            ...
            1,   127, 3,   2,   1,  0, 7, 9, 1, 5,  101, 110, 116,
            114, 121, 0,   0,   10, 6, 1, 4, 0, 65, 0,   11};
        WasmEngine eng;
        auto mod = eng.load(std::vector<uint8_t>(wasm, wasm + sizeof(wasm)));
        auto inst = eng.instantiate(
            mod, 1000000, 64 * 1024,
...
int main(int argc, char *argv[]) {
  ...
  if (argc >= 4 && std::string(argv[1]) == "l2" &&
      std::string(argv[2]) == "vm" && std::string(argv[3]) == "selftest") {
    static const uint8_t wasm[] = {0,  97, 115, 109, 1,   0,   0,   0,   1, 5,
                                   1,  96, 0,   1,   127, 3,   2,   1,   0, 7,
                                   9,  1,  5,   101, 110, 116, 114, 121, 0, 0,
                                   10, 6,  1,   4,   0,   65,  0,   11};
    WasmEngine eng;
    auto mod = eng.load(std::vector<uint8_t>(wasm, wasm + sizeof(wasm)));
    auto inst = eng.instantiate(
  ...
  if (argc >= 4 && std::string(argv[1]) == "dao-submit") {
    ...
    if (DAO::createProposal(prop)) {
      if (amt > 0)
        PolicyManager::recordSpend(from, amt);
