...
void Miner::startMiningProcess(
    const std::string &minerAddress) { // ✅ FIX: Use Miner::
  try {
    std::cout << "🚀 Starting mining process for: " << minerAddress
              << std::endl;
    if (!miningActive.exchange(true)) {
      ...
      while (miningActive) {
        ...
        std::vector<Transaction> pendingTx =
        ...
        std::string minerDilithiumKeyPath = "keys/miner_dilithium.pub";
        std::string minerFalconKeyPath = "keys/miner_falcon.pub";
        Block minedBlock =
