...
use crate::air::{EvaluationFrame, crate::proof::TraceInfo};
use alyn_math::StarkField;
use crate::alyn_utils::uninit_vector;
#[cfg(feature = "concurrent")]
use crate::alyn_utils::{iterators::*, rayon};
...
impl<B: StarkField> TraceTable<B> {
    ...
    pub fn with_meta(width: usize, length: usize, meta: Vec<u8>) -> Self {
        let info = crate::proof::TraceInfo::with_meta(width, length, meta);
        assert!(
    ...
    pub fn init(columns: Vec<Vec<B>>) -> Self {
        ...
        let info = crate::proof::TraceInfo::with_meta(columns.len(), trace_length, Vec::new());
...
impl<B: StarkField> Trace for TraceTable<B> {
    ...
    fn info(&self) -> &crate::proof::TraceInfo {
        &self.info
    }
