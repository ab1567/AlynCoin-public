...
use alyn_math::{StarkField, ToElements};
...
pub struct crate::proof::TraceInfo {
...
impl crate::proof::TraceInfo {
    ...
    /// Creates a new [crate::proof::TraceInfo] from the specified trace width and length.
    ///
    ...
    /// Creates a new [crate::proof::TraceInfo] from the specified trace width, length, and metadata.
    ///
    /// An execution trace described by this trace info is limited to a single segment.
    ///
    /// # Panics
    ...
    /// Creates a new [crate::proof::TraceInfo] with main and auxiliary segments.
    ///
    /// # Panics
    /// Panics if:
    ...
    pub fn new_multi_segment(
        ...
        assert!(
            full_width <= crate::proof::TraceInfo::MAX_TRACE_WIDTH,
            "total number of columns in the trace cannot be greater than {}, but was {}",
            crate::proof::TraceInfo::MAX_TRACE_WIDTH,
            full_width
        );
        ...
        assert!(
            num_aux_segment_rands <= crate::proof::TraceInfo::MAX_RAND_SEGMENT_ELEMENTS,
            "number of random elements required by a segment cannot exceed {}, but was {}",
            crate::proof::TraceInfo::MAX_RAND_SEGMENT_ELEMENTS,
            num_aux_segment_rands
        );

        crate::proof::TraceInfo {
            main_segment_width,
            aux_segment_width,
            num_aux_segment_rands,
            trace_length,
...
impl<E: StarkField> ToElements<E> for crate::proof::TraceInfo {
...
impl Serializable for crate::proof::TraceInfo {
...
impl Deserializable for crate::proof::TraceInfo {
    /// Reads [`crate::proof::TraceInfo`] from the specified `source` and returns the result.
    ///
    /// # Errors
    /// Returns an error of a valid [`crate::proof::TraceInfo`] struct could not be read from the specified
    /// `source`.
    fn read_from<R: ByteReader>(source: &mut R) -> Result<Self, DeserializationError> {
        ...
        if full_trace_width >= crate::proof::TraceInfo::MAX_TRACE_WIDTH {
        ...
        } else if num_aux_segment_rands > crate::proof::TraceInfo::MAX_RAND_SEGMENT_ELEMENTS {
            return Err(DeserializationError::InvalidValue(format!(
                "number of random elements required by a segment cannot exceed {}, but was {}",
                crate::proof::TraceInfo::MAX_RAND_SEGMENT_ELEMENTS,
                num_aux_segment_rands
        ...
        if trace_length < crate::proof::TraceInfo::MIN_TRACE_LENGTH.ilog2() as u8 {
            return Err(DeserializationError::InvalidValue(format!(
                "trace length cannot be smaller than 2^{}, but was 2^{}",
                crate::proof::TraceInfo::MIN_TRACE_LENGTH.ilog2(),
                trace_length
            )));
        }
...
mod tests {
    ...
    use super::{ToElements, crate::proof::TraceInfo};
    ...
    fn trace_info_to_elements() {
        ...
        let info = crate::proof::TraceInfo::new(main_width, trace_length as usize);
        ...
        let info = crate::proof::TraceInfo::new_multi_segment(
            main_width as usize,
            aux_width as usize,
            aux_rands,
